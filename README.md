...
3. **Configurar el servicio Systemd (para que corra 24/7):**
   Edita el archivo `trading_bot.service` y asegúrate de que las rutas (como `WorkingDirectory` y `ExecStart`) coincidan con la ubicación donde clonaste el repo. Por defecto apunta a `/opt/pairs_trading_bot`.
...
4. **Verificar los logs:**
   ```bash
   sudo journalctl -u trading_bot.service -f
   ```
